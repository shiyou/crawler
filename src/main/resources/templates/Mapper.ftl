...
<mapper namespace="com.crawler.po.${rawObject.name}">
	<resultMap type="com.crawler.po.${rawObject.name}" id="${rawObject.name?uncap_first}">
	...
	<select id="select" resultType="com.crawler.po.${rawObject.name}">
		SELECT 
		<#list rawFields as field>
	...
	<delete id="delete" parameterType="com.crawler.po.${rawObject.name}">
	...
	<select id="selectList" parameterType="String" resultMap="${rawObject.name}">
		...
		FROM ${rawObject.tablesTbl.tableName}
	</select>
